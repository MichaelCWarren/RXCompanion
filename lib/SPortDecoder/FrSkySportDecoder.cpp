...
FrSkySportDecoder::FrSkySportDecoder()
{
}

void FrSkySportDecoder::begin(FrSkySportSensor *sensor1, FrSkySportSensor *sensor2, FrSkySportSensor *sensor3,
...
uint16_t FrSkySportDecoder::decode(uint8_t byte)
{
	uint16_t result = SENSOR_NO_DATA_ID;
	return this->processByte(byte);
}

uint16_t FrSkySportDecoder::processByte(uint8_t byte)
{
	uint16_t result = SENSOR_NO_DATA_ID;
	...
	else
	{
		...
		else if ((state == DATA_FRAME) && ((byte == FRSKY_SENSOR_DATA_FRAME) || (byte == FRSKY_SENSOR_EMPTY_FRAME)))
		{
			if (byte == FRSKY_SENSOR_DATA_FRAME)
			{
				crc = byte;
				state = APP_ID_BYTE_1;
			} // If data frame found, initialize the CRC and start collecting APP ID
			else
			{
				hasStuffing = false;
				state = START_FRAME;
			} // If empty frame found, skip the frame and start looking for next one
		}
